...
public class LoginMenu : MonoBehaviour {
    ...
    {
        UserJSON userJSON = new UserJSON { kills = 0, deaths = 0 };
        string userStr = JsonUtility.ToJson(userJSON);
        IEnumerator e = DCF.RegisterUser(playerUsername, playerPassword, userStr); // << Send request to register a new user, providing submitted username and password. It also provides an initial value for the data string on the account, which is "Hello World".
        while (e.MoveNext())
        {
            yield return e.Current;
        }
